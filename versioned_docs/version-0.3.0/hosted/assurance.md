...
| `urn:sag:acr:mfa` | A method-neutral requirement; on this hosted gateway, an upstream that reported multi-factor |

The first three are ordered, so asking for `federated` is met by a
`federated-mfa` sign-in. `urn:sag:acr:mfa` is a requirement, not an `acr`
returned in a token. Asking for a value SAG does not know is an error, not a
shrug. Self-hosted Node deployments can also meet the method-neutral
requirement with [local MFA](../self-host/local-identities.md).
...
There is one thing SAG will not take an upstream's word for: a multi-tenant
Microsoft or Google endpoint asserting an address. Any organisation the
provider federates can reach such an endpoint, and a directory administrator
sets their own users' attributes, so an operator has to say which tenants an
endpoint like that is for, or let the provider state per sign-in that it
verified the domain of the address. Until one of the two is true, sign-ins
through it are refused rather than trusted. The reasoning is in
[ADR 0019](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/adr/0019-a-common-upstream-must-bound-what-it-may-assert.md),
and the settings are in the
