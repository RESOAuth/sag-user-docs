...
sidebar_position: 8
description: "How to move between SAG releases, including the local identity and release verification changes in 0.3.0."
---

# Upgrading

The core gateway keeps sessions in encrypted browser values, so an upgrade is
usually a new build reading the same environment variables. If you enable
local identities in 0.3.0, their credential files also need persistent storage
and backups. What changes between releases is the set of variables, and
occasionally what a default means.
...
Verify the release, pin its authenticated image digest, then deploy it. See
...
Existing upstream and email-code deployments do not need a credential
migration. Check these changes before enabling new features:

1. **Local identities are opt-in and Node-only.** The `file` backend needs
   Node.js 24.7.0 or newer, one process writing a persistent private
   directory, an explicit non-rotating `SUBJECT_SALT`,
   `LOCAL_IDENTITY_DOMAINS`, and a state store. Read
   [Local identities](local-identities.md) before provisioning a record.
2. **Local sign-in does not verify an email address.** A local password, TOTP,
   or backup-code sign-in can return `email` without `email_verified`. A local
   identity uses a stable record id for `sub`, so linking an existing upstream
   account to it can change the `sub` your relying party sees. Check both
   claims in your application's account-linking logic.
3. **Protect and migrate local credentials deliberately.** Local TOTP seeds
   are plaintext in private credential files. Back them up securely. If you
   tried the unreleased sealed-TOTP draft, convert or replace those seeds
   offline before using 0.3.0; see the
   [TOTP migration guidance](local-identities.md#totp-and-backup-codes).
4. **Verify a published release before deploying it.** The release now has a
   signed manifest and provenance for its source and container. Use the
   [release verification](release-verification.md) steps to approve and pin
   the image digest. Pulling a tag alone does not check its signature.

`urn:sag:acr:mfa` is a new method-neutral request: local or federated MFA can
meet it, while the returned `acr` names the method actually used. See
[Configuration](../reference/configuration.md#authentication-context).
