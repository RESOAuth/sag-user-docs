...
sidebar_position: 1
description: "Everything is an environment variable, so the same build runs unchanged on Cloudflare Workers, AWS Lambda, a container and a laptop. Two rules shape all..."
---
...
[Relying parties](relying-parties.md) and [Upstreams](upstreams.md).
...
| `SAG_SECRET` | a well-known development value | Master secret. 48 random bytes, unique to this issuer. Protects sessions, transactions, codes, and retained upstream refresh credentials. Local TOTP seeds are plaintext and do not use it <sup>[1](#sealed-environment-variables)</sup> |
| `SAG_SECRET_PREVIOUS` | - | The secret being retired, so a rotation does not sign everybody out and the local identity rekey can open durable credentials. See [Operations](../self-host/operations.md) |
...
<sup>1</sup> Worth sealing rather than setting in plain text: see
[Sealed environment variables](#sealed-environment-variables).
...
[Multi-region](../self-host/multi-region.md): listing a peer here trusts its keys as
...
| `PEER_JWKS_CACHE_BACKEND` | `memory` | `memory`, `cf-kv`, `dynamodb`. `memory` does not survive an isolate or container restart, so a peered deployment should set one of the other two and is warned in the log if it does not - see [Multi-region](../self-host/multi-region.md) |
...
[ADR 0011](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/adr/0011-subject-derived-from-the-verified-address.md) and
[ADR 0023](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/adr/0023-node-flat-file-local-identities.md).
...
See [State and limits](../self-host/state-and-limits.md) for what it is for and which
backend to pick.
...
Lambda cannot. See [Local identities](../self-host/local-identities.md) before enabling
...
[test/local-stack/](../self-host/local-stack.md) is a worked example: an
...
| `OTP_TTL` | `600` | |
| `OTP_MAX_ATTEMPTS` | `5` | Per transaction, and see the honesty note in [Limitations](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/limitations.md) |
...
See [Profile claims](profile-claims.md) for what is relayed, what is guessed,
...
See [ADR 0024](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/adr/0024-method-neutral-mfa-requirement.md).
