...
sidebar_position: 1
description: "Two ways to get an instance running, both with no configuration at all."
---
...
[Docker](docker.md) for the details and
[Configuration](../reference/configuration.md) for what can go in it.
...
To add Microsoft or Google, see [Upstreams](../reference/upstreams.md). To add your own
application, see [Relying parties](../reference/relying-parties.md).

## Optional local sign-in

On a self-hosted Node process or container, you can provision local password
and MFA identities for selected email domains. This needs Node.js 24.7.0 or
newer, one process writing a persistent private directory, an explicit
`SUBJECT_SALT`, and a state store for attempt limits. Cloudflare Workers and
Lambda do not support this backend.

Set `LOCAL_IDENTITIES_BACKEND=file` and `LOCAL_IDENTITY_DOMAINS`, then create
records with the `generate-local-identity` tool. Start with
[Local identities](local-identities.md) for the configuration, record
provisioning, backup, and relying-party claim rules. The container setup is in
[Docker](docker.md#local-identities).
