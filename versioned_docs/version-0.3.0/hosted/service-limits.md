...
The free hosted gateway is shared. We may slow or pause an application's
traffic if it opens a large number of connections or more than 100 people
sign in through it in an hour. People may see a message asking them to wait.
See the [terms of service](./terms-of-service.md), or
[get in touch](../support/index.md) if you expect that level of use.
...
These are properties of the hosted gateway:
...
- **No user database.** No profile storage, no groups, no roles. The hosted gateway tells you
...
- **What the hosted gateway holds.** No password or user database. A session is an
