...
With the `email` scope you get `email`. The hosted gateway also returns
`email_verified: true` when the address has been verified. Check for the
claim rather than treating a missing value as true.
