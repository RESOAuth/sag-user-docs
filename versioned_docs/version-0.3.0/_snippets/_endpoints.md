...
| `/token` | POST, OPTIONS | Exchange an authorisation code for an `id_token`. Answers cross-origin, so a browser may call it |
| `/userinfo` | GET, POST, OPTIONS | Claims about the signed-in person, using the access token. Answers cross-origin too |
