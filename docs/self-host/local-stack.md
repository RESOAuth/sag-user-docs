---
sidebar_label: "Local stack"
sidebar_position: 7
description: "Every platform SAG runs on, at once, on one machine."
---
...
[Multi-region](./multi-region.md), exercised for real rather
than only in `test/peer-jwks.test.js`'s unit tests.

## Who is signing in

Four applications, all running the same hundred lines from
[examples/relying-party](https://github.com/RESOAuth/smart-access-gateway/blob/main/examples/relying-party/server.js):
...
[Profile claims](../reference/profile-claims.md) for why. The applications
...
[localstack/init/10-provision.sh](https://github.com/RESOAuth/smart-access-gateway/blob/main/test/local-stack/localstack/init/10-provision.sh): an
...
[localstack/clients](https://github.com/RESOAuth/smart-access-gateway/tree/main/test/local-stack/localstack/clients). That script is the same three
resources [Deployment](./deployment.md) asks for on AWS, which
...
| --- | --- | --- |
| Settings shared by the Node and Lambda instances | [env/sag-common.env](https://github.com/RESOAuth/smart-access-gateway/blob/main/test/local-stack/env/sag-common.env) | `./stack.sh restart sag-node` |
| AWS coordinates | [env/aws.env](https://github.com/RESOAuth/smart-access-gateway/blob/main/test/local-stack/env/aws.env) | `./stack.sh restart sag-lambda` |
| The Node instance's relying parties | [clients/](https://github.com/RESOAuth/smart-access-gateway/tree/main/test/local-stack/clients) | within `CLIENTS_STORE_CACHE_TTL`, 60s by default; a newly added one within ten |
| The Lambda instance's relying parties | [localstack/clients/](https://github.com/RESOAuth/smart-access-gateway/tree/main/test/local-stack/localstack/clients) | `podman restart sag-localstack`, which re-seeds the bucket |
| The Workers instance, all of it | [workers/wrangler.dev.toml](https://github.com/RESOAuth/smart-access-gateway/blob/main/test/local-stack/workers/wrangler.dev.toml) | `./stack.sh restart sag-workers` |
| The self-describing client's registration | nothing to edit - it is [the document the application serves](https://github.com/RESOAuth/smart-access-gateway/blob/main/examples/relying-party/server.js) | `./stack.sh restart rp-cimd`, then within `CLIENTS_CIMD_CACHE_TTL` |
