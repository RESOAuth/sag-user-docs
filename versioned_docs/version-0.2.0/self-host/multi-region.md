...
sidebar_position: 4
description: "One issuer can be served by more than one running instance - several AWS regions, several clouds, or both - provided every instance answers the same..."
---
...
[Configuration](../reference/configuration.md)'s terms, `SAG_ISSUER`. It is the only
...
request. [config.js](https://github.com/RESOAuth/smart-access-gateway/blob/main/src/config.js) only falls back to the request's own
host when `SAG_ISSUER` is unset at all, which is refused outright once a real
hostname is in play (see [Deployment](deployment.md)). Every other place
that matters - the `iss` claim, the discovery documents, `config.issuer` in
[context.js](https://github.com/RESOAuth/smart-access-gateway/blob/main/src/context.js) - reads that one fixed value. A deployment
...
sets a `Domain` attribute (see [session.js](https://github.com/RESOAuth/smart-access-gateway/blob/main/src/session.js)), so a cookie
...
- **`SAG_SECRET` (and `SAG_SECRET_PREVIOUS` during rotation).** Identical
  ...
  [Operations](operations.md) stricter, not different: step 1 has to land
...
- **The signing key material.** Does not have to be identical any more - see
  ...
  This is where [Post-quantum](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/post-quantum.md) and
  [Limitations](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/limitations.md) stop being hypothetical: AWS KMS does not
...
- **`SUBJECT_SALT`.** Identical, and - as
  [Operations](operations.md) already says for a single deployment - never
...
- **The state store, if configured.** [State and limits](state-and-limits.md)
  ...
  - **Genuine multi-cloud.** [store/index.js](https://github.com/RESOAuth/smart-access-gateway/blob/main/src/store/index.js) signs its
...
[ADR 0009](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/adr/0009-peer-jwks-federation.md) for why this was chosen over
...
"a day is safe" is the figure [Operations](operations.md) already gives
...
- **`cf-kv`**. The right Cloudflare-native answer, and notably *not* the
  choice state store made: [State and limits](state-and-limits.md) refuses
...
- **`/healthz`** is deliberately the harder question: can this instance
  ...
  [Operations](operations.md) for how to read what it reports.
