...
sidebar_position: 7
description: "Runbooks for the things an operator has to do occasionally and must not get wrong. Each one states what breaks if it is done in the wrong order."
---
...
migration - see
[ADR 0014](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/adr/0014-sealed-values-remain-independent-of-the-issuer.md).
...
2. **Rekey retained upstream refresh credentials.** Skip this step when no
   ...
   [Local identities](local-identities.md#disable-replace-back-up-and-restore).
...
[ADR 0011](https://github.com/RESOAuth/smart-access-gateway/blob/main/docs/adr/0011-subject-derived-from-the-verified-address.md). Turning
...
- **Signing key leaked.** Configure the new key and make it primary in one
  ...
  **On a peered deployment, do not take the compromised instance offline.**
  ...
  [Multi-region](multi-region.md)). It cannot tell an outage from a
...
failover check at it - see [Multi-region](multi-region.md).
...
- `peer_jwks` - only present with `PEER_JWKS_URLS` set. A peer with
  ...
  not list an instance's key" - see [Multi-region](multi-region.md).
...
[test/local-stack/verify.js](https://github.com/RESOAuth/smart-access-gateway/blob/main/test/local-stack/verify.js) does.
