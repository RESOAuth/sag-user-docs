...
to their email address. The hosted gateway does not hold passwords or run a
user database. A self-hosted Node deployment can also offer
[local identities](./self-host/local-identities.md).
...
container, or a plain Node process from the same code. Its core needs no
database; local identities are an optional, file-backed Node feature.
...
To provision local password and MFA sign-in on Node or in a container, read
[Local identities](./self-host/local-identities.md).
...
| [Local identities](./self-host/local-identities.md) | Operator-provisioned password and MFA sign-in on one Node instance |
